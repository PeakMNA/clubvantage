...
const mockCaddies: Caddy[] = [
  ...
  { id: 'c0000001-0000-0000-0000-000000000004', name: 'Wichai Khamwan', skillLevel: 'advanced', status: 'OFF_DUTY', experience: 6 },
...
function mapToBackendPlayerType(playerType: 'MEMBER' | 'GUEST' | 'DEPENDENT' | 'WALK_UP'): BackendPlayerType {
...
export default function GolfPage() {
  ...
  useEffect(() => {
    ...
    if (scheduleConfig) {
      ...
      // Use default schedule config as fallback
      effectiveConfig = { ...DEFAULT_SCHEDULE_CONFIG, id: `default-${selectedCourse}`, courseId: selectedCourse }
    }
            ...
            for (const p of booking.players) {
              if (p && p.position >= 1 && p.position <= 4) {
                ...
                slots[p.position - 1] = {
                  ...
                  cartStatus: ((p as any).cartStatus || 'NONE') as RentalStatus,
                  caddyStatus: ((p as any).caddyStatus || 'NONE') as RentalStatus,
    ...
    setFlights(generatedFlights)
      ...
      console.error('[Golf Page] Error generating flights:', error)
    }
  ...
  const teeSheetMetrics = useMemo(() => {
    ...
    const totalPlayers = flights.reduce((acc: number, f: Flight) => acc + f.players.filter((p: Player | null) => p !== null).length, 0)
    const checkedInPlayers = flights.reduce((acc: number, f: Flight) => acc + f.players.filter((p: Player | null) => p?.checkedIn).length, 0)
    const onCoursePlayers = flights.filter((f: Flight) => f.status === 'STARTED').reduce((acc: number, f: Flight) => acc + f.players.filter((p: Player | null) => p !== null).length, 0)
    ...
    return {
      ...
      projectedRevenue: totalPlayers * 2500,
      cartsAssigned: carts.filter(c => c.status === 'IN_USE').length,
  ...
  const handleBookTeeTime = useCallback(async (data: {
    slotId: string
    players: Array<{ id: string; name: string; type: 'MEMBER' | 'GUEST' | 'DEPENDENT' | 'WALK_UP'; memberId?: string }>
    notes?: string
    ...
    const apiPlayers = data.players.map((player, index) => ({
      ...
      guestName: player.type === 'GUEST' || player.type === 'WALK_UP' ? player.name : undefined,
    }))
    ...
    try {
      ...
      if (data.cartId) {
        setCarts(prev => prev.map(cart =>
          cart.id === data.cartId
            ? { ...cart, status: 'IN_USE' as Cart['status'], currentAssignment: selectedFlight?.time }
            : cart
        ))
      }
  ...
  const handleRowNoShow = useCallback((flight: Flight) => {
    ...
    setFlights((prev: Flight[]) => prev.map((f: Flight) =>
      f.id === flight.id
        ? { ...f, status: 'NO_SHOW' as FlightStatus }
        : f
    ))
  }, [])
  ...
  // Build tee sheet content (optionally wrapped with POS)
          ...
          <div className="space-y-6">
            ...
            {viewMode === 'day' && layoutType === 'list' && (
              <TeeSheetBookingView
                ...
                isLoading={isCoursesLoading || isScheduleConfigLoading || isTeeSheetLoading}
  ...
  // Conditionally wrap tee sheet content with POS when enabled
  const renderedTeeSheetContent = (() => {
    if (isPOSEnabled) {
  ...
  return (
    <div className="p-6 space-y-6">
      <PageHeader
        ...
        breadcrumbs={[{ label: 'Golf' }, { label: 'Tee Sheet' }]}
        actions={
            <Button onClick={() => {
      ...
      <BookingModal
        ...
        availableCaddies={caddies.filter(c => c.status === 'AVAILABLE').map(c => {
          ...
          return {
            ...
            status: c.status as 'AVAILABLE' | 'ASSIGNED' | 'OFF_DUTY',
          }
        })}
