  ...
  return (
    <div className="px-5 py-6 pb-36 space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-[22px] font-semibold text-stone-900">Unbilled Activity</h1>
          {activity.sinceDate && (
      ...
      {activity.categories.length > 0 ? (
        ...
        <div className="rounded-xl border border-stone-100 bg-white p-8 text-center">
          <p className="text-sm text-stone-400">No unbilled charges</p>
        </div>
      )}
      ...
      {activity.payments.length > 0 && (
        <section>
          <h2 className="text-base font-semibold text-stone-900 mb-3">Payments Received</h2>
          <div className="rounded-xl border border-stone-100 bg-white divide-y divide-stone-100">
            {activity.payments.map((payment) => (
              <div key={payment.id} className="flex items-center gap-3 px-4 py-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-stone-500">{format(payment.date, 'MMM d')}</p>
                </div>
      ...
      <div className="rounded-xl border-2 border-stone-200 bg-stone-50 p-4">
        <div className="flex items-center justify-between">
          <p className="text-sm font-semibold text-stone-700">Net Unbilled</p>
          <p
            className={cn(
              'text-xl font-bold tracking-tight',
              activity.netUnbilled > 0 ? 'text-stone-900' : 'text-emerald-600'
            )}
          >
